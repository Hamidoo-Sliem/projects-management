...
class AdminController extends Controller
{
    ...
    static public function _projectsDetails() {
       $q = Project::with('status')->latest()->get();
        return view('backoffice.admin._projectsdetails',['rows' => $q]);
        // return $q;
    }

    static public function _getCurStatus($projectID):string{
       $current = \DB::table('project_transaction_history AS PTH')
       ->join('transaction_history AS TH','TH.id','=','PTH.status_id')
       ->select('title')
       ->where('is_done','1')->where('project_id',$projectID)
       ->orderBy('PTH.id','desc')->first();

       return $current->title ?? "";
    }

    static public function _getPrevStatus($projectID):string{
        $previous = \DB::table('project_transaction_history AS PTH')
        ->join('transaction_history AS TH','TH.id','=','PTH.status_id')
        ->select('title')
        ->where('is_done','1')->where('project_id',$projectID)
        ->orderBy('PTH.id','desc')->skip(1)->first();
 
        return $previous->title ?? "";
     }

     static public function _getNexStatus($projectID):string{
        $next = \DB::table('project_transaction_history AS PTH')
        ->select('status_id')
        ->where('is_done','0')->where('project_id',$projectID)
        ->orderBy('PTH.id','desc')->first();
            $nexStatus = \App\Models\TransactionHistory::find($next->status_id)->previous;
        return $nexStatus ?? "";
     }
     
     static public function _getRFlags($projectID) {
        $flags="";
        $rf = ProjectRedFlag::selectRaw('client_id,title,status,type,DATE(created_at) AS date')
              ->where('project_id',$projectID)
              ->latest()->get();
         foreach($rf as $v):
           $client = User::find($v->client_id)->name;
           $link = url('project/followup/'.$projectID.'#kt_project_redflags_tab');
           $typ = ($v->type == 'approved') ? '<span class="badge badge-light-success fw-bold me-auto px-4 py-3">موافق</span>' : '<span class="badge badge-light-danger fw-bold me-auto px-4 py-3">مرفوض</span>' ;
           if($v->status == 'new'):
            $status = "<span class='badge badge-light-info fw-bold me-auto px-4 py-3'>جديد</span>";
            elseif($v->status == 'inprogress'):
             $status = "<span class='badge badge-light-primary fw-bold me-auto px-4 py-3'>جارى الرد</span>" ;
            else:
                $status = "<span class='badge badge-light-success fw-bold me-auto px-4 py-3'>إكتمل</span>" ;
            endif;
           $flags .= "<tr class='fw-bold fs-7 text-white'>
     ...
     static public function _getObstacles($projectID) {
        $obstacles="";
        $ob = \App\Models\ProjectObstacle::selectRaw('user_id,title,message,type_id,is_close,DATE(created_at) AS date')
              ->where('project_id',$projectID)
              ->latest()->get();
         foreach($ob as $v):
           $user = User::find($v->user_id)->name;
           $link = url('obstacle/projects/'.$projectID);
           $close = ($v->is_close == 1) ? '<span class="badge badge-light-success fw-bold me-auto px-4 py-3">تم الاغلاق</span>' : '<span class="badge badge-light-danger fw-bold me-auto px-4 py-3">لم يغلق</span>' ;
           $obstacles .= "<tr class='fw-bold fs-7 text-white'>
           <td><a href='{$link}' target='_blank'>{$v->title}</a></td>
           <td>{$v->message}</td>
           <td>{$v->type_id}</td>
           <td>{$user}</td>
