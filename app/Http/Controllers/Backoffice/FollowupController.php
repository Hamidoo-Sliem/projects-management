...
class FollowupController extends Controller
{
    ...
    public function _changeProjectFiles(Request $request)
    {
        ...
        try {
            ...
            DB::commit();
            $this->COMMON_HELPER->_sendEmailStating($request['project_id'],"تعديل فى مرفقات المشروع",3);
    ...
    public function _addReqFile(Request $req)
    {
        ...
        ProjectRequirements::where('id', $req['req_id'])
            ...
            $usersWithRoles= \App\Models\User::with('roles')->get();
             ...
             endforeach;
                $projectTitle = Project::find($req['project_id'])->title;
                $url = url('client/projects/' . $req['project_id']);
                $typ = 'reply';
            try {
                $sendMail = Mail::to($users)->send(new \App\Mail\RequirementEmail($projectTitle,$url,$typ));
            }catch(\Exception $e){
                return response()->json(["MSG" => $e->getMessage()]);
            }
        /* } else {
    ...
    public function _changeProjectDetails(Request $request)
    {
        ...
        try {
            ...
            DB::commit();
            $this->COMMON_HELPER->_sendEmailStating($request['projectID'],"تعديل فى تفاصيل المشروع",3);
            return response()->json(["MSG" => "success"]);
        } catch (\PDOEXception $e) {
            DB::rollBack();
            return response()->json(["MSG" => $e->getMessage()]);
        ...
        try {
            if ($request->has('accountType') && $request['accountType'] == 'kashef') {
                ...
                if (ProjectKashefAccounts::where('project_id', $request['projectID'])->count()) {
                    ...
                    $this->COMMON_HELPER->_sendEmailStating($request['projectID'],"تعديل فى تفاصيل الاستمارة للمشروع (kashef)",4.1);
                } else {
                    $arr = [
            ...
            if ($request->has('accountType') && $request['accountType'] == 'survey') {
                ...
                if (ProjectSurveyAccounts::where('project_id', $request['projectID'])->count()) {
                    ...
                    $this->COMMON_HELPER->_sendEmailStating($request['projectID'],"تعديل فى تفاصيل الاستمارة للمشروع (Survey)",4.1);
                } else {
                    $arr = [
                        'project_id' => $request['projectID'],
                        'url' => $request['URL'],
    ...
    public function _changeProjectEquipment(Request $request)
    {
        ...
        try {
            ...
            DB::commit();
            $eType = DB::table('equipment_types')->where('id',$request['eqType'])->first()->title;
            $this->COMMON_HELPER->_sendEmailStating($request['projectID'],"تعديل فى {$eType} للمشروع",4);
            return response()->json(["MSG" => "success"]);
        } catch (\PDOEXception $e) {
            DB::rollBack();
            return response()->json(["MSG" => $e->getMessage()]);
    ...
    public function _changeProjectFinance(Request $request)
    {
        ...
        try {
            ...
            endforeach;
            DB::commit();
            $this->COMMON_HELPER->_sendEmailStating($request['projectID'],"تعديل فى تسعيرات الفرق البحثية للمشروع",3);
            return response()->json(["MSG" => "success"]);
        } catch (\PDOEXception $e) {
            DB::rollBack();
            return response()->json(["MSG" => $e->getMessage()]);
