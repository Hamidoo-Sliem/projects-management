...
class ClientController extends Controller
{
    ...
    public function _addNewRequirement(Request $req)
    {
        ...
        if (empty($err)) {
            ...
            endforeach;
                $projectTitle = Project::find($req['project'])->title;
                $url = url('project/followup/' . $req['project']);
                $typ = 'new';
            try {
    ...
    public function _storeRedflag(Request $req)
    {
        ...
        if (! (empty($req['redflag']))) {
            ...
            endforeach;
                ...
                $url = url('project/followup/' . $req['project_id']);
                $typ = 'new';
            try {
                $sendMail = Mail::to($users)->send(new \App\Mail\RedflagEmail($projectTitle,$url,$typ));
            }catch(\Exception $e){
                return response()->json(["MSG" => $e->getMessage()]);
            }
