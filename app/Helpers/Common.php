...
class Common
{
    ...
    public function _sendEmailStating($projectId,$type=null,$toUsers=null) {
        $users = [];
        $usersWithRoles= \App\Models\User::with('roles')->get();
        switch ($toUsers) :
            case 3 :
        foreach ($usersWithRoles as $user) :
            if($user->roles[0]->id == 1 || $user->roles[0]->id == 3 || $user->roles[0]->id == 5) :
                $users[]= $user->email;
            endif;
        endforeach;
         break;
         case 4 :
            foreach ($usersWithRoles as $user) :
                if($user->roles[0]->id == 1 || $user->roles[0]->id == 3 || $user->roles[0]->id == 5 || $user->roles[0]->id == 9) :
                    $users[]= $user->email;
                endif;
            endforeach;
             break;
             case 4.1 :
                foreach ($usersWithRoles as $user) :
                    if($user->roles[0]->id == 1 || $user->roles[0]->id == 3 || $user->roles[0]->id == 5 || $user->roles[0]->id == 4) :
                        $users[]= $user->email;
                    endif;
                endforeach;
                 break;
        ...
        endswitch;
            $mailData = [
                'project_title' => Project::find($projectId)->title,
                'changeType' => $type
            ];
            #check mail send or not
            try {
                $sendMail = Mail::to($users)->send(new \App\Mail\DetectProjectStating($mailData));
            }catch(\Throwable $e){
